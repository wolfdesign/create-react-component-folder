...
function createFiles(componentName, componentPath, cssFileExt) {
  return new Promise((resolve) => {
    // File extension
    const ext = config.hasFlag('typescript') ? 'ts' : 'js';
    const jsxExt = 'js';
    let name = componentName;
    const isJsxFile = config.hasFlag('jsx');
    const componentFileName = `${name}.${isJsxFile ? jsxExt : ext}x`;
    // file names to create
    const files = [componentFileName];
    ...
    if (config.hasFlag('stories')) {
      files.push(`${name}.story.${ext}`);
    }

    // Add Styles for styled-components
    if (config.hasFlag('styled')) {
      files.push(`${name}.styled.${ext}`);
    }

    // Add css | less | sass file if desired
    if (cssFileExt && !config.hasFlag('reactnative') && !config.hasFlag('styled')) {
    ...
    fs.createDirectorys(componentPath)
      .then(() => {
        ...
        for (let i = 0; i < files.length; i += 1) {
          ...
          if (file === `${componentName}.graphql`) {
              promises.push(fs.writeFileAsync(filePath, ''));
            } else if (file.indexOf(`${name}.${ext}`) > -1 || file.indexOf(`${name}.${jsxExt}`) > -1 || file === `index.${jsxExt}`) {
              promises.push(
          ...
          if ((file.indexOf(`story.${ext}`) || file.indexOf(`story.${ext}`)) > -1) {
            promises.push(
          ...
          } else if ((file.indexOf(`styled.${ext}`) || file.indexOf(`styled.${ext}`)) > -1) {
            promises.push(
              createReactComponentStyled(name).then((template) =>
                // eslint-disable-next-line implicit-arrow-linebreak
                fs.writeFileAsync(
                  filePath,
                  formatPrettier(template, prettierParser, prettierOptions),
