...
function createFiles(componentName, componentPath, cssFileExt) {
  return new Promise((resolve) => {
    ...
    const componentFileName = `${componentName}.${isJsxFile ? jsxExt : ext}`;
      ...
      .then(() => {
        ...
        for (let i = 0; i < files.length; i += 1) {
          ...
          if (file === `${componentName}.graphql`) {
            ...
            } else if (file === `${name}.${ext}` || file === `index.${ext}` || file === `${name}.${jsxExt}` || file === `index.${jsxExt}`) {
              promises.push(
