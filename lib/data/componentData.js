...
/**
 * Get the styled-component template
 *
 * @returns {Promise|Promise<string>}
 */
async function getStyledTemplate() {
  try {
    return await readFileAsync(`${config.getValue('templates')}/styles.handlebars`);
  } catch (error) {
    return readFileAsync(`${config.getValue('crcf')}/templates/styles.handlebars`);
  }
}
...
/**
 * Creates styled-component for the React component
 *
 * @param {String} componentName - Component name
 * @returns {String}
 */
 async function createReactComponentStyled(componentName) {
  const name = stringHelper.componentNameWithoutSpecialCharacter(componentName);
  const file = getStyledTemplate();
  const template = Handlebars.compile(await file);
